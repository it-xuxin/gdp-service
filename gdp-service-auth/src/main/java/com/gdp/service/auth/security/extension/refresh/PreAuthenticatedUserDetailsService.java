package com.gdp.service.auth.security.extension.refresh;
...
public class PreAuthenticatedUserDetailsService<T extends Authentication> implements AuthenticationUserDetailsService<T>, InitializingBean {
     ...
     *
     * @see com.gdp.service.auth.security.config.AuthorizationServerConfig#tokenServices(AuthorizationServerEndpointsConfigurer)
     */
    ...
    public UserDetails loadUserDetails(T authentication) throws UsernameNotFoundException {
        ...
        AuthIdentityEnum authenticationIdentityEnum = IBaseEnum.getEnumByValue(RequestUtils.getAuthenticationIdentity(), AuthIdentityEnum.class);
        UserDetailsService userDetailsService = userDetailsServiceMap.get(clientId);
        if (clientId.equals(SecurityConstants.APP_CLIENT_ID)) {
            // 移动端的用户体系是会员，认证方式是通过手机号 mobile 认证
            AppUserDetailsServiceImpl memberUserDetailsService = (AppUserDetailsServiceImpl) userDetailsService;
